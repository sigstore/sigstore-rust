...
#[test]
fn test_v03_github_attestation_no_tlog_entries() {
    // Real bundle captured from GitHub's artifact attestation API for
    // jdx/communique@v0.1.9. It is a v0.3 bundle with an RFC3161 timestamp
    // and no tlog entries — the shape that motivated PR #80.
    let bundle =
    ...
    // Default validation still requires tlog entries.
    assert!(
        validate_bundle(&bundle).is_err(),
        "default validation should reject a bundle with no tlog entries"
    );
    ...
    assert!(
        result.is_ok(),
        "GitHub attestation bundle should validate when tlog verification is disabled: {:?}",
        result.err()
    );
}
